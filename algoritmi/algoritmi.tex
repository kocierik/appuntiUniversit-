...
\subsection{Dijkstra}
Determina i cammini di \textbf{costo minimo} da singola sorgente nel caso in cui tutti gli archi abbiano costo $\geq 0$.\\
\begin{itemize}
    \item Sia $G = (V, E)$ un grafo orientato con funzione costo $w$.
    \begin{itemize}
        \item I costi degli archi devono essere $\geq 0$. 
    \end{itemize}
    \item Sia $T$ una parte dell'albero dei cammini di costo minimo radicato in $s$
    \begin{itemize}
        \item $T$ rappresenta porzioni di cammini di costo minimo che partono da $s$.
    \end{itemize}
    \item Allora l'arco $(u, v)$ con $u \in V(T)$ e $v \not\in V(T)$ che
    minimizza la quantità $d_s_u + w(u, v)$ appartiene ad un cammino minimo da $s$ a $v$.
\end{itemize}
...
\begin{center}
    \includegraphics[width=9cm]{images/algoritmoDijkstra.png}
\end{center}

 \subsubsection{Costo computazionale:}
 \begin{itemize}
     \item L'inizializzazione ha costo $O(n)$.
     \item Le operazioni $find()$ e $deleteMin()$ hanno costo $O(log n)$ e sono eseguite al più $n$ volte.
        \begin{itemize}
         \item Una volta che un nodo è stato estratto dalla coda di priorità
        non verrà più reinserito.
     \end{itemize}
     \item Le operazioni $insert()$ e $decreaseKey()$ hanno costo
     $O(log n)$ e sono eseguite al più $m $volte.
     \begin{itemize}
         \item Una volta per ogni arco.
     \end{itemize}
     \item Totale: $O( (n+m)\hspace{0.1cm} log \hspace{0.1cm} n) = O(m \hspace{0.1cm} log \hspace{0.1cm} n)$ se tutti i nodi sono raggiungibili dalla sorgente.
 \end{itemize}
...
\begin{itemize}
    \item Si può applicare a grafi orientati con costi arbitrari (anche negativi), purché non ci siano cicli negativi
    \item Sia $V = \{1, 2, … n\}$
    \item Sia $D_x_y^k$ la distanza minima dal nodo $x$ al nodo $y$, nell'ipotesi in cui gli eventuali nodi intermedi possano appartenere esclusivamente all'insieme $\{1, … k\}$
    \item La soluzione al nostro problema è $D_x_y^n$ per ogni coppia di nodi $x$ e $y$.
\end{itemize}
 
 $$D_x_y^k=min\{ D_x_y^k^-^1 , D_x_k^k^-^1 + D_k_y^k^-^1 \}$$

\newpage

\textbf{Esempio:}
\begin{center}
    \includegraphics[width=9cm]{images/esempioS.png}
\end{center}
\textbf{Costo computazionale:}\\
$$O(n^3)$$

\subsection{Algoritmo di FloydWarshall}

\begin{center}
    \includegraphics[width=12cm]{images/floyd.png}
\end{center}

\subsection{Ricostruzione dei cammini}
\begin{itemize}
        \item Per ricostruire i cammini di costo minimo possiamo usare una matrice dei successori $next[x,y]$ di $n * n$ elementi.
    \begin{itemize}
    \item next[x,y] è l'indice del secondo nodo attraversato dal cammino di costo minimo che va da x a y (il primo nodo di tale cammino è x, l'ultimo è y).
    \end{itemize}
\end{itemize}

Se non sono presenti pesi negativi utilizzare \textbf{Dijkstra}, altrimenti utilizzare \textbf{Bellman Ford}.


\subsection{Teoria della NP-completezza}
\textbf{P} indica la complessità polinomiale.\\
\begin{itemize}
    \item Consideriamo un problema Q come una relazione:
    $$Q \subseteq< I x S$$
    \begin{itemize}
        \item I è l'insieme delle istanze di ingresso.
        \item S è l'insieme delle soluzioni.
    \end{itemize}
    \item Possiamo immaginare $Q$ come un predicato che, dato
    in ingresso una istanza di input $x \in I$ e una soluzione
    $s \in S$, restituisce:
    \begin{itemize}
        \item 1 se $(x,s) \in Q$ ($s$ è soluzione del problema $Q$ sull'istanza $x$)
        \item 0 altrimenti ($s$ non è soluzione del problema $Q$ sull'istanza $x$)
    \end{itemize}
\end{itemize}

\begin{itemize}
    \item Data una funzione $f(n)$, chiamiamo $TIME(f(n))$
    (risp. $SPACE(f(n))$ ) l'insieme di tutti i problemi
    decisionali che possono essere risolti in tempo
    (risp. in spazio) $O(f(n))$.
    \item \textbf{TIME} indica un insieme di problemi
\end{itemize}
...
\begin{itemize}
    \item La \textbf{classe P} è la classe dei problemi risolvibili in \textbf{tempo polinomiale} nella dimensione $n$ dell'istanza di ingresso:
    $$ P = \cup^\infty_c_=_0 \hspace{0.1cm} TIME(n^c)$$
    \item La \textbf{classe PSPACE} è la classe dei problemi risolvibili in \textbf{spazio polinomiale} nella dimensione n dell'istanza di ingresso:
    $$ PSPACE = \cup^\infty_c_=_0 \hspace{0.1cm} SPACE(n^c)$$
    \item La \textbf{classe EXPTIME} è la classe dei problemi risolvibili in \textbf{tempo esponenziale} nella dimensione $n$ dell'istanza di ingresso:
    $$ EXPTIME = \cup^\infty_c_=_0 \hspace{0.1cm} TIME(2 \textsuperscript{n^c})$$
\end{itemize}

Se un problema è in $P$ allora esegue una quantità di operazioni polinomiali.

\begin{itemize}
    \item Un algoritmo che richiede tempo polinomiale riuscirà al più ad accedere ad un numero polinomiale di locazioni di memoria diverse, quindi:
    $$ P \subseteq PSPACE$$
    \item Poiché $n^c$ locazioni di memoria possono trovarsi al più in $2^{ n^c }$ stati diversi, si ha anche:
    $$PSPACE \subseteq EXPTIME$$
    \item Non è noto se le inclusioni di cui sopra sono strette (non si sa se $P \subset PSPACE$ o se $PSPACE \subset EXPTIME$), ma una delle due inclusioni è stretta! (in quanto si sa che $P \subset EXPTIME$)
\end{itemize}
\newpage
\subsubsection{Verificare vs Certificare}
\begin{itemize}
    \item Come visto in precedenza, nei problemi di decisione siamo interessati a sapere se una istanza x del problema verifica una certa proprietà.
    \item Spesso però siamo anche interessati a conoscere un qualche oggetto y, che dipende da x e dal problema da risolvere, che possa \textbf{certificare} il fatto che $x$ \textbf{gode di tale proprietà}. 
    \item \textbf{certificatore divergente:} Nel caso in cui non goda di tale proprietà il programma andrebbe in \textbf{loop} infinito.
\end{itemize}

Informalmente \textbf{NP} è la classe dei \textbf{problemi decisionali} che ammettono \textbf{certificati verificabili} in tempo polinomiale.\\

Un algoritmo decisionale \textbf{non deterministico}, invece, oltre alle normali istruzioni può eseguire istruzioni del tipo "\textbf{indovina} $Z \in S$".

\begin{itemize}
    \item Usando il non determinismo si può risolvere il problema della soddisfacibilità in tempo lineare.
    \item Nota: Un algoritmo non deterministico può essere rappresentato da un \textbf{albero di decisione}. l'algoritmo restituisce 1 se c'è almeno una foglia che restituisce 1.
\end{itemize}

\begin{center}
    \includegraphics[width=8cm]{images/classeOP.png}
\end{center}

La \textbf{classe NP} è la classe dei problemi risolvibili in \textbf{tempo polinomiale} non deterministico nella dimensione n dell'istanza di ingresso:
$$ NP = \cup^\infty_c_=_0 \hspace{0.1cm} NTIME(n^c)$$
\newpage
\subsubsection{Gerarchia della complessità}
\begin{center}
    \includegraphics[width=8cm]{images/gerarchia.png}
\end{center}
Delle inclusioni qui sotto \textbf{almeno una} è propria:
$$P \subseteq NP \subseteq PSPACE \subseteq EXPTIME$$

La \textbf{classe NP completa}, indica i problemi \textbf{più difficili} della classe $NP$. Questi tipi di problemi restituiscono un valore \textbf{True} o \textbf{False}.

\subsubsection{riducibilità polinomiale}
\begin{itemize}
    \item La soddisfacilità di espressioni booleane è riducibile polinomialmente nella verifica di verità di \textbf{formule booleane quantificate}.
    \begin{itemize}
        \item Consideriamo l'espressione booleana $E$ che contiene le variabili $x_1, .. x_n$
        \item Consideriamo $f$ tale che restituisce una formula booleana quantificata così definita: $f(E) = \exists x_1. .. .\exists x_n. E$
        \item Tale trasformazione ha costo lineare e abbiamo che:\\
        $E$ è soddisfacibile se e solo se $f(E) = \exists x_1. .. .\exists x_n. E$ è vera.
    \end{itemize}
\end{itemize}

\subsubsection{Implicazioni della riducibilità polinomiale}
Effettuo una riduzione polinomiale per rendere risolvibile il problema:
\begin{center}
    \includegraphics[width=8cm]{images/implicazioneRiducibilita.png}
\end{center}

\subsection{NP completezza}
\begin{itemize}
    \item Un problema decisionale $Q$ si dice \textbf{NP-arduo} se ogni problema $W \in NP$ è riducibile polinomialmente a $Q$.
    \item Un problema decisionale Q si dice \textbf{NP-completo} se appartiene alla classe NP ed è NP-arduo.
    \item \textbf{Nota:} se un qualunque problema decisionale $NP$ completo appartenesse alla classe $P$, allora $P = NP$:
    \begin{itemize}
        \item \textbf{Sarebbe un disastro!}
        \begin{itemize}
            \item Il problema della \textbf{decifratura} di un documento crittografato sarebbe polinomiale (quindi eseguibile in tempi “ragionevoli”).
            \item Infatti, se l'algoritmo di \textbf{cifratura} (polinomiale con chiave di cifratura) è noto, allora esiste un certificato polinomiale per il problema della decifratura: la password di cifratura!
        \end{itemize}
    \end{itemize}
\end{itemize}
Il \textbf{problema della fermata limitata} verifica se un programma termina in $k$ passi.

\subsection{Teorema di Cook}
\begin{itemize}
    \item Il problema \textbf{SAT} (soddisfacibilità di espressioni booleane in forma normale congiuntiva) è \textbf{NP-completo}.
    \item \textbf{Corollario:} Dato un problema $Q$, se esiste una riduzione di \textbf{SAT} in $Q$, allora possiamo concludere che $Q$ è un problema \textbf{NP-arduo}.
\end{itemize}
